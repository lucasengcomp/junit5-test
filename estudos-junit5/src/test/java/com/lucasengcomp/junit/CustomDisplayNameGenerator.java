...
public class CustomDisplayNameGenerator implements DisplayNameGenerator {
    ...
    private String formatCase(String input) {
        ...
        for (int i = 1; i < input.length(); i++) {
            start = verifyCharacterInitial(input, i, words, start);
        }
        words.append(input.substring(start));
        String formattedString = words.toString().trim();

        formattedString = formattedStringToLowerCase(formattedString);
    ...
    private static int verifyCharacterInitial(String input, int i, StringBuilder words, int start) {
